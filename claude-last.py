...
class PermissionAnalyzer:
    ...
    def get_file_info(self, path):
        try:
            ...
            risk_level, risk_types = self.check_security_risk(perms, st.st_uid, path_obj.suffix.lower())
    ...
    def check_security_risk(self, perms, uid, extension):
        risks = []
        risk_level = 'none'
        ...
        try:
            for root, dirs, files in os.walk(path, topdown=True, followlinks=follow_symlinks):
                ...
                for name in dirs + files:
                    ...
                    rows.append(self.format_row(info))
                    with self.lock:
                        self.total_files += 1
