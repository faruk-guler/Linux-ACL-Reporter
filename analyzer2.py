...
"""
    ...
    def create_json_data(self, rows_data, scanned_directory):
        """HTML satırlarından JSON verisi oluşturur."""
        data = {
            'summary': {
        ...
        for row in rows_data:
            ...
            try:
                ...
                tr = soup.find('tr')
                if tr and 'error-row' in tr.get('class', []):
                    continue
                tds = tr.find_all('td')
                if len(tds) >= 11:
                    file_data = {
                        'path': tds[0].get_text(strip=True),
                        'type': tds[1].get_text(strip=True).replace('📁', '').replace('📄', '').replace('🔗', '').strip(),
                        'permissions': tds[2].get_text(strip=True),
                        'octal': tds[3].get_text(strip=True),
                        'owner': tds[4].get_text(strip=True),
                        'group': tds[5].get_text(strip=True),
                        'size': tds[6].get_text(strip=True),
                        'modified': tds[7].get_text(strip=True),
                        'risk': tds[8].get_text(strip=True),
                        'symlink_target': tds[9].get_text(strip=True),
                        'acl': tds[10].get_text(strip=True)
                    }
                    data['data'].append(file_data)
            except Exception as e:
                self.logger.warning(f"Error parsing table row for JSON: {e}")
        return data
    ...
    def generate_html_report(self, rows_data, output_file, scanned_directory):
        ...
        json_data = self.create_json_data(rows_data, scanned_directory)
...
<body>
    ...
    def main(self):
        ...
        if args.json_output:
            try:
                data = self.create_json_data(rows, args.path)
                with open(args.json_output, 'w', encoding='utf-8', errors='replace') as f:
                    json.dump(data, f, indent=2, default=lambda o: str(o) if isinstance(o, (datetime.datetime,)) else o)
